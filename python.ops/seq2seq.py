...
def _extract_argmax_and_embed(embedding, output_projection=None,
  ...
  def loop_function(prev, _):
    ...
    #prev_symbol shpae [batch_size, n_hidden] * [n_hidden, num_decoder_symbols]=[batch_size,num_decoder_symbols]
...
def attention_decoder(decoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    # decoder_inputs list n_step*[batch_size,embedding_size]
    batch_size = array_ops.shape(decoder_inputs[0])[0]  # Needed for reshaping.
    attn_length = attention_states.get_shape()[1].value
    # attention_state shape [batch_size, n_step, n_hidden]
    # attn_length=n_steps
    ...
    # hidden=[batch_size,n_step,1,n_hidden] 
    hidden_features = []
    v = []
    attention_vec_size = attn_size  # Size of query vectors for attention.
    # attention_vec_size shape [n_hidden]
    # num_heads=1
    for a in xrange(num_heads):
      k = variable_scope.get_variable("AttnW_%d" % a,
                                      [1, 1, attn_size, attention_vec_size])
      # k shape [1,1,n_hidden,n_hidden]
      # convoluation after: [batch_size,n_step,1,n_hidden]
    ...
    def attention(query):
      ...
      for a in xrange(num_heads):
        with variable_scope.variable_scope("Attention_%d" % a):
          ...
          # y shape [batch_size,attention_vec_size]
          y = array_ops.reshape(y, [-1, 1, 1, attention_vec_size])
          # Attention mask is a softmax of v^T * tanh(...).
          # hidden_features= list len(hidden_fetures)=num_heads shape [batch]
...
def embedding_attention_decoder(decoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    emb_inp = [
        embedding_ops.embedding_lookup(embedding, i) for i in decoder_inputs]
    # emb_inp len(decoder_inputs)*[batch_size,embedding_size]
    # emb_inp 51*[batch_size,1024]
...
def embedding_attention_seq2seq(encoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    # encoder_outpus shape n_step*[batch_size,n_hidden]
    # encoder_inputs tuple(state1,state2,state3)
    # First calculate a concatenation of encoder outputs to put attention on.
    top_states = [array_ops.reshape(e, [-1, 1, cell.output_size])
                  for e in encoder_outputs]

    # top_states shape n_step*[batch_size,1,n_hidden]
    attention_states = array_ops.concat(1, top_states)
    # attention_states shape [batch_size,n_step,n_hidden]
